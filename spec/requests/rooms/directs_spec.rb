...
RSpec.describe 'Rooms::Directs', type: :request do
  describe 'POST /rooms/directs' do
    let(:alice) { create :user }
    let(:bob) { create :user }
    let(:room_direct_params) { { user_id: bob.id } }

    context 'when not signed in' do
      it 'returns a 302 response' do
        post rooms_directs_path, params: room_direct_params
    ...
    context 'when signed in and room_direct_params is self' do
      let(:self_room_direct_params) { { user_id: alice.id } }

      before { sign_in alice }

      it 'returns a 302 response' do
        post rooms_directs_path, params: self_room_direct_params
        expect(response.status).to eq 302
      end
    ...
    context 'when signed in and room_direct_params is other_user, room exists' do
      before do
    ...
    context 'when signed in and room_direct_params is other_user, room does not exist' do
      before { sign_in alice }
